...
class AdsPackagesBlocListener extends StatelessWidget {
  const AdsPackagesBlocListener({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocListener<UserAddPackageCubit, AdsPackagesState>(
      ...
      listener: (context, state) {
        state.whenOrNull(
          ...
          success: (adspackageResponse) async {
          context.pop();
            
            Constants.showToast(msg: adspackageResponse.message);
          },
          error: (error) {
            setupErrorState(
              context,
